    ...
    sections: [
      {
        ...
        content: `일반 안내
다음 안내는 귀하가 이 웹사이트를 방문할 때 귀하의 개인정보에 어떤 일이 일어나는지에 대한 간단한 개요를 제공합니다. 개인정보란 귀하를 개인적으로 식별할 수 있는 모든 데이터를 말합니다. 개인정보 보호에 관한 자세한 정보는 아래에 제시된 본 개인정보 처리방침에서 확인하실 수 있습니다.

이 웹사이트에서의 데이터 수집
이 웹사이트에서의 데이터 처리는 누가 책임지나요?
이 웹사이트에서의 데이터 처리는 웹사이트 운영자가 담당합니다. 운영자의 연락처는 본 개인정보 처리방침의 “책임자에 대한 안내” 항목에서 확인하실 수 있습니다.

우리는 어떻게 귀하의 데이터를 수집하나요?
한편으로 귀하의 데이터는 귀하가 우리에게 직접 제공할 때 수집됩니다. 예를 들어, 문의 양식에 입력하는 데이터가 이에 해당합니다.
...
우리는 귀하의 데이터를 어떤 목적으로 사용하나요?
일부 데이터는 웹사이트를 오류 없이 제공하기 위해 수집됩니다. 다른 데이터는 이용자의 방문 행태 분석에 사용될 수 있습니다. 웹사이트를 통해 계약이 체결되거나 준비될 수 있는 경우, 전송된 데이터는 계약 제안, 주문 또는 기타 주문 관련 문의 처리를 위해서도 처리됩니다.

귀하는 귀하의 데이터와 관련하여 어떤 권리를 가지나요?
귀하는 언제든지 무료로 귀하의 저장된 개인정보의 출처, 수신자 및 처리 목적에 대해 정보 제공을 요청할 권리가 있습니다. 또한 이러한 데이터를 정정하거나 삭제할 것을 요구할 권리가 있습니다. 귀하가 데이터 처리에 동의하신 경우, 향후 효력을 발생하도록 언제든지 동의를 철회할 수 있습니다. 또한 특정 요건 하에서 귀하의 개인정보 처리 제한을 요구할 권리가 있습니다. 아울러 관할 감독 기관에 불만을 제기할 권리도 있습니다.
이 외에도 개인정보 보호와 관련된 기타 모든 질문에 대해 언제든지 저희에게 문의하실 수 있습니다.

분석 도구 및 제3자 도구
귀하가 이 웹사이트를 방문할 때 귀하의 이용 행태가 통계적으로 분석될 수 있습니다. 이는 주로 이른바 분석 프로그램을 사용하여 이루어집니다.
이러한 분석 프로그램에 대한 자세한 내용은 아래의 개인정보 처리방침에서 확인하실 수 있습니다.`
      },
      {
        title: "2. 호스팅",
        content: `당사는 다음 제공업체를 통해 웹사이트 콘텐츠를 호스팅합니다.

외부 호스팅
이 웹사이트는 외부에서 호스팅됩니다. 이 웹사이트에서 수집된 개인정보는 호스팅 제공업체의 서버에 저장됩니다. 여기에는 주로 IP 주소, 문의 내용, 메타 및 통신 데이터, 계약 데이터, 연락처 정보, 이름, 웹사이트 접속 데이터 및 웹사이트를 통해 생성되는 기타 데이터가 포함될 수 있습니다.
...
미국`
      },
      {
        ...
        content: `개인정보 보호
이 페이지의 운영자는 귀하의 개인정보 보호를 매우 중요하게 생각합니다. 당사는 귀하의 개인정보를 기밀로 취급하며, 관련 법률 및 본 개인정보 처리방침에 따라 처리합니다.
...
책임자에 대한 안내
이 웹사이트에서의 데이터 처리를 담당하는 책임자는 다음과 같습니다.
Handokhelper
Michaelstraße 26
...
보관 기간
본 개인정보 처리방침에서 구체적인 보관 기간이 명시되지 않은 경우, 귀하의 개인정보는 해당 데이터 처리 목적이 더 이상 적용되지 않을 때까지 당사에 보관됩니다. 귀하가 정당한 삭제 요청을 하거나 데이터 처리에 대한 동의를 철회하는 경우, 당사는 귀하의 개인정보를 삭제합니다. 다만 세법 또는 상법상 보존 의무 등 귀하의 개인정보를 계속 보관해야 하는 다른 법적 사유가 있는 경우에는 그러하지 않으며, 이러한 사유가 더 이상 존재하지 않을 때 삭제가 이루어집니다.

이 웹사이트에서의 데이터 처리에 대한 법적 근거에 관한 일반 정보
귀하가 데이터 처리에 동의한 경우, 당사는 GDPR 제6조 제1항 (a)호 및 제9조 제2항 (a)호에 근거하여 귀하의 개인정보를 처리합니다(제9조 제1항의 특별한 범주의 데이터가 처리되는 경우). 개인정보를 제3국으로 전송하는 데 대해 명시적인 동의를 하신 경우, 데이터 처리는 또한 GDPR 제49조 제1항 (a)호에 근거하여 이루어집니다. 귀하가 쿠키 저장 또는 귀하의 단말기(예: 디바이스 핑거프린팅)에 대한 정보 접근에 동의한 경우, 데이터 처리는 추가적으로 TDDDG 제25조 제1항에 근거합니다. 동의는 언제든지 철회할 수 있습니다. 귀하의 데이터가 계약 이행 또는 계약 체결 전 조치 이행을 위해 필요한 경우, 당사는 GDPR 제6조 제1항 (b)호에 근거하여 귀하의 데이터를 처리합니다. 또한 당사는 법적 의무를 이행하기 위해 데이터 처리가 필요한 경우 GDPR 제6조 제1항 (c)호에 근거하여 데이터를 처리합니다. 그 밖에 데이터 처리는 당사의 정당한 이익에 근거하여 GDPR 제6조 제1항 (f)호에 따라 이루어질 수 있습니다. 각 처리 상황에서 적용되는 구체적인 법적 근거는 아래 각 항목에서 별도로 안내합니다.

개인정보 수신자
당사는 사업 활동 수행 과정에서 여러 외부 기관과 협력합니다. 이 과정에서 개인정보가 이러한 외부 기관에 전달되는 경우가 있습니다. 당사는 계약 이행에 필요한 경우, 법적으로 의무가 있는 경우(예: 세무 당국에 대한 데이터 제공), GDPR 제6조 제1항 (f)호에 따른 정당한 이익이 있는 경우 또는 다른 법적 근거가 허용하는 경우에만 개인정보를 외부 기관에 전달합니다. 수탁 처리자를 이용하는 경우, 당사는 유효한 데이터 처리 계약에 근거하여서만 고객의 개인정보를 전달합니다. 공동으로 처리를 수행하는 경우에는 공동 책임자에 관한 계약을 체결합니다.

동의 철회권
많은 데이터 처리 절차는 귀하의 명시적인 동의가 있어야만 가능하며, 귀하는 이미 부여한 동의를 언제든지 철회할 수 있습니다. 철회 이전에 이루어진 데이터 처리의 적법성은 철회의 영향을 받지 않습니다.

특별한 상황에서의 데이터 처리 및 직접 마케팅에 대한 반대권 (GDPR 제21조)
...
관할 감독 기관에 대한 불만 제기권
GDPR 위반 시, 정보주체는 특히 상시 거주지, 직장 소재지 또는 의심되는 위반 장소가 속한 회원국의 감독 기관에 불만을 제기할 권리가 있습니다. 불만 제기권은 다른 행정적 또는 사법적 구제수단에 영향을 주지 않습니다.

데이터 이동권
귀하는 동의 또는 계약 이행을 근거로 자동 처리되는 데이터를 일반적으로 사용되는 기계 판독 가능한 형식으로 본인 또는 제3자에게 제공받을 권리가 있습니다. 다른 책임자에게 데이터의 직접 전송을 요구하는 경우, 이는 기술적으로 가능한 범위에서만 이루어집니다.

열람, 정정 및 삭제
적용 법령의 범위 내에서 귀하는 언제든지 귀하의 저장된 개인정보, 그 출처와 수신자, 데이터 처리 목적에 대한 무료 정보 제공 및 필요한 경우 해당 데이터의 정정 또는 삭제를 요구할 권리를 가집니다. 개인정보와 관련된 기타 질문이 있는 경우에도 언제든지 저희에게 문의하실 수 있습니다.

처리 제한권
귀하는 귀하의 개인정보 처리 제한을 요구할 권리가 있습니다. 이를 위해 언제든지 저희에게 연락하실 수 있습니다. 처리 제한권은 다음의 경우에 해당합니다.
...
SSL 또는 TLS 암호화
본 사이트는 보안을 위해, 그리고 귀하가 사이트 운영자인 우리에게 전송하는 주문이나 문의와 같은 기밀 콘텐츠의 전송을 보호하기 위해 SSL 또는 TLS 암호화를 사용합니다. 암호화된 연결은 브라우저의 주소 표시줄이 “http://”에서 “https://”로 변경되고, 브라우저 표시줄에 자물쇠 아이콘이 표시되는 것으로 확인할 수 있습니다.
SSL 또는 TLS 암호화가 활성화된 경우, 귀하가 당사로 전송하는 데이터는 제3자가 읽을 수 없습니다.

광고 이메일에 대한 거부
법적 고지 의무(인프레숨)를 통해 공개된 연락처 정보는 명시적으로 요청하지 않은 광고 및 정보 자료를 발송하는 데 사용하는 것을 금지합니다. 이 페이지의 운영자는 스팸 이메일과 같은 원치 않는 광고 정보의 발송 시 법적 조치를 취할 권리를 명시적으로 보유합니다.`
      },
      {
        title: "4. 이 웹사이트에서의 데이터 수집",
        content: `문의 양식
귀하가 문의 양식을 통해 문의를 보내는 경우, 당사는 문의 양식에 기재된 정보 및 귀하가 제공한 연락처 정보를 문의 처리 및 후속 문의 대응 목적으로 저장합니다. 이 데이터는 귀하의 동의 없이 제3자에게 제공되지 않습니다.
...
이메일, 전화 또는 팩스를 통한 문의
귀하가 이메일, 전화 또는 팩스로 문의하는 경우, 귀하의 문의 내용과 그와 관련하여 발생하는 모든 개인정보(이름, 문의 내용)는 귀하의 요청 처리를 위해 당사에 저장 및 처리됩니다. 이 데이터는 귀하의 동의 없이 제3자에게 제공되지 않습니다.
이 데이터 처리는 귀하의 요청이 계약 이행과 관련이 있거나 계약 전 조치 수행에 필요한 경우 GDPR 제6조 제1항 (b)호에 근거하여 이루어집니다. 그 밖의 모든 경우에는 우리에게 제기되는 문의를 효과적으로 처리하기 위한 정당한 이익( GDPR 제6조 제1항 (f)호) 또는 귀하의 동의( GDPR 제6조 제1항 (a)호, 요청된 경우)에 근거합니다. 동의는 언제든지 철회할 수 있습니다.
귀하가 문의를 통해 당사에 전송한 데이터는 귀하가 삭제를 요청하거나 저장에 대한 동의를 철회하거나 데이터 저장 목적이 더 이상 적용되지 않을 때(예: 귀하의 요청 처리가 완료된 경우)까지 당사에 보관됩니다. 법정 보존 기간 등 강행 법규는 이에 영향을 받지 않습니다.`
      },
      {
        title: "5. 뉴스레터",
        content: `뉴스레터 데이터
귀하가 웹사이트에서 제공하는 뉴스레터를 수신하고자 하는 경우, 우리는 귀하의 이메일 주소와 더불어 귀하가 해당 이메일 주소의 소유자이며 뉴스레터 수신에 동의한다는 사실을 확인할 수 있는 정보가 필요합니다. 그 밖의 데이터는 수집되지 않거나, 자발적으로 제공하는 경우에만 수집됩니다. 이 데이터는 요청한 정보 발송에만 사용되며, 제3자에게 제공되지 않습니다.
...
본 사이트는 글꼴을 통일성 있게 표시하기 위해 Google에서 제공하는 Google Fonts를 사용합니다. 페이지를 호출하면, 브라우저는 텍스트와 글꼴을 올바르게 표시하기 위해 필요한 글꼴을 브라우저 캐시에 로드합니다.
...
The following notes provide a simple overview of what happens to your personal data when you visit this website. Personal data is all data by which you can be personally identified. Detailed information on the subject of data protection can be found in our privacy policy set out below.
...
Part of the data is collected to ensure the error-free provision of the website. Other data can be used to analyze your user behavior. If contracts can be concluded or initiated via the website, the data transmitted is also processed for contract offers, orders or other order-related inquiries.
...
External Hosting
This website is hosted externally. The personal data collected on this website is stored on the servers of the host(s). This may primarily include IP addresses, contact requests, meta and communication data, contract data, contact details, names, website access data and other data generated via a website.
...
If you send us inquiries via the contact form, the information you provide in the inquiry form, including the contact details you enter there, will be stored by us for the purpose of processing the inquiry and in the event of follow‑up questions. We do not pass on this data without your consent.
