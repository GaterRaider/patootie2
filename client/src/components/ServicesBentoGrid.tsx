...
export function ServicesBentoGrid({ services, onSelect, language }: ServicesBentoGridProps) {
    ...
    const ServiceCardContent = ({ service }: { service: ServiceItem }) => (
        <Card
            className="h-full cursor-pointer group hover:shadow-2xl transition-all duration-300 border-2 hover:border-indigo-500/50 relative z-10 overflow-hidden bg-card/50 backdrop-blur-sm flex flex-col hover:-translate-y-2 hover:scale-[1.02]"
